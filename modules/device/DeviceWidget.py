...
# Stelle sicher, dass die kompilierte UI-Datei im selben Ordner liegt
# (oder passe den Import-Pfad an)
try:
    from .ui_DeviceWidget import Ui_Dialog
except ImportError:
    print("Fehler: Konnte 'ui_DeviceWidget.py' nicht finden.")
    print("Bitte die 'ui_DeviceWidget.ui' Datei mit 'pyside6-uic' kompilieren:")
    print("pyside6-uic modules/device/ui_DeviceWidget.ui -o modules/device/ui_DeviceWidget.py")
    # Fallback, damit die Anwendung nicht crasht, wenn die UI fehlt
...
# (Passe die Pfade ggf. an deine Projektstruktur an)
...
class DeviceWidget(QWidget, Ui_Dialog):
    ...
    def __init__(self, device_manager: DeviceManager, log_manager: LogManager, parent=None):
